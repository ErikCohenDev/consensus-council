...
export const DebateRound: React.FC<DebateRoundProps> = ({
  ...
  return (
    <div aria-label={`debate-round-${roundNumber}`} className="border rounded p-3">
      <div className="font-medium mb-2">Debate Round {roundNumber}{status ? ` • ${status}` : ''}</div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <div className="text-sm font-semibold mb-1">Consensus Points ({emergingConsensus.length})</div>
