...
import { useConnectionState, useUIState, useAppActions } from '@/stores/appStore'

const StatusDot = ({ status }: { status: 'connecting' | 'connected' | 'disconnected' | 'error' }) => {
  const color =
    ...
    <div className="min-h-screen flex">
      <aside className="w-56 border-r border-gray-200 dark:border-gray-800 p-4">
        ...
        <nav className="flex flex-col gap-2">
          ...
          <NavLink to="/audit" className={({ isActive }) => (isActive ? 'font-semibold' : '')}>Audit</NavLink>
          <NavLink to="/council" className={({ isActive }) => (isActive ? 'font-semibold' : '')}>Council</NavLink>
          <NavLink to="/pipeline" className={({ isActive }) => (isActive ? 'font-semibold' : '')}>Pipeline</NavLink>
