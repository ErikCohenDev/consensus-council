...
const mapCouncilMembers = (rawMembers: unknown): CouncilMemberInfo[] => {
  ...
  return rawMembers.map((m: any) => ({
    memberId: String(m.role ?? crypto.randomUUID?.() ?? `member_${Math.random().toString(36).slice(2, 8)}`),
...
const transformPipeline = (raw: any) => {
  ...
  return {
    documents,
    councilMembers: Array.isArray(raw.council_members) ? mapCouncilMembers(raw.council_members) : [],
    currentDebateRound,
    researchProgress,
