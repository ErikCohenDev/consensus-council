...
import { CheckIcon, UsersIcon, XIcon, DollarSignIcon, InfoIcon, FileTextIcon, AlertTriangleIcon } from 'lucide-react'
...
// Provider gating based on keys present in Settings
...
const availableAuditors: AuditorRole[] = [
	...
	},
	{
		...
		suggestedModels: [
			{ provider: 'Google', model: 'gemini-1.5-pro', description: 'Strong analytical capabilities' },
...
export const CouncilPage = () => {
	...
	const isProviderEnabled = useCallback((providerLabel: string) => {
		const keyName = providerKeyMap[providerLabel]
		return Boolean(keyName && apiKeys[keyName])
	}, [apiKeys])

	hasLLMProviders = useMemo(
		() => Object.keys(providerKeyMap).some(provider => isProviderEnabled(provider)),
		[isProviderEnabled]
	)

	hasTavily = Boolean(apiKeys.tavily)

	toggleAuditor = (auditorId: string) => {
		...
		updateAuditorModel = (auditorId: string, model: string) => {
			setConfig(prev => ({
				...prev,
		...
		updateAuditorPersonality = (auditorId: string, personality: string) => {
			setConfig(prev => ({
				...prev,
		...
		generateFullPrompt = (auditor: AuditorRole, personality: string) => {
...
Focus areas: ${auditor.specialization}`
	...
	selectedAuditorData = selectedMember ? availableAuditors.find(a => a.id === selectedMember) : null
	...
	useEffect(() => {
		...
		estimateModelCost = (modelKey: string, inTokens: number, outTokens: number, numRounds: number) => {
		...
		totalEstimatedCost = config.selectedAuditors.reduce((sum, auditorId) => {
	...
	const enabledModels = useMemo(() => allModelOptions.filter(model => isProviderEnabled(model.provider)), [isProviderEnabled]);
			...
			<div className="flex-1 flex flex-col">
				...
				{/* Council Grid */}
					<div className="flex-1 p-6">
						{!hasLLMProviders && (
							<div className="mb-6 border rounded-lg p-4 bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-700 text-yellow-900 dark:text-yellow-100">
								<div className="flex items-center gap-3">
									...
									<div className="flex-1">
										...
										<p className="text-sm opacity-90">
											Please add API keys for one or more LLM providers in the Settings page to enable the council.
										</p>
									</div>
								</div>
							</div>
						...
						<div className="mb-6 border rounded-lg p-4 bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-700">
							...
							<div className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-2">
								<div>
									...
									<input type="number" min={1} value={rounds} onChange={(e) => setRounds(Number(e.target.value) || 1)} className="w-full border rounded px-2 py-1 text-sm bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600" />
								</div>
								<div>
									<div className="text-xs text-gray-600 dark:text-gray-400 mb-1">Input tokens / round</div>
									<input type="number" min={0} value={tokensInPerRound} onChange={(e) => setTokensInPerRound(Math.max(0, Number(e.target.value) || 0))} className="w-full border rounded px-2 py-1 text-sm bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600" />
								</div>
								<div>
									<div className="text-xs text-gray-600 dark:text-gray-400 mb-1">Output tokens / round</div>
									<input type="number" min={0} value={tokensOutPerRound} onChange={(e) => setTokensOutPerRound(Math.max(0, Number(e.target.value) || 0))} className="w-full border rounded px-2 py-1 text-sm bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600" />
								</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
							{config.selectedAuditors.map((auditorId) => {
								...
								if (!auditor) return null
									...
									const personalityData = personalityTypes.find(p => p.id === personality)
									const perMemberCost = model ? estimateModelCost(model, tokensInPerRound, tokensOutPerRound, rounds) : 0
								...
								return (
									<div key={auditorId} className="border rounded p-3 bg-white dark:bg-blue-800 border-blue-300 dark:border-blue-600">
										...
										<div className="text-xs text-blue-600 dark:text-blue-400 font-mono mb-1">{model}</div>
											<div className="flex items-center justify-between">
												<div className="text-xs text-gray-600 dark:text-gray-300">{personalityData?.name}</div>
												<div className="text-xs text-emerald-700 dark:text-emerald-300 font-medium">${perMemberCost.toFixed(2)}</div>
											</div>
											<button
												type="button"
					...
					<div className="grid grid-cols-2 lg:grid-cols-3 gap-4 h-fit">
						{availableAuditors.map((auditor) => {
							...
							const isResearchAgent = auditor.id === 'research'
							const isDisabled = !hasLLMProviders || (isResearchAgent && !hasTavily)

							return (
								<button
									key={auditor.id}
									type="button"
									onClick={() => !isDisabled && setSelectedMember(auditor.id)}
									disabled={isDisabled}
									className={`relative border-2 rounded-lg p-4 transition-all ${isSelected 
										? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' 
										: 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800' 
									} ${isDisabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:shadow-md hover:border-gray-300 dark:hover:border-gray-600'}`}
								>
									{/* Selection Checkbox */}
					...
					<div className="flex-1 p-4 space-y-6">
						...
						<div>
							...
							<div>
								...
								<select
									value={config.auditorPersonalities[selectedAuditorData.id] || selectedAuditorData.defaultPersonality}
								...
								<p className="text-xs text-gray-500 mt-1">
									{personalityTypes.find(p => p.id === (config.auditorPersonalities[selectedAuditorData.id] || selectedAuditorData.defaultPersonality))?.description}
								</p>
							</div>
							...
							<div>
								...
								>
									...
									{selectedAuditorData.suggestedModels
										.filter(opt => isProviderEnabled(opt.provider))
										.map((modelOption) => (
								...
								<div className="text-xs text-gray-500 mt-1 space-y-1">
									<div>
										{selectedAuditorData.suggestedModels.find(m => m.model === (config.auditorModels[selectedAuditorData.id] || selectedAuditorData.defaultModel))?.description || 'Selected from catalog'}
									</div>
									<div className="text-gray-600 dark:text-gray-300">
										{(() => {
											const m = MODEL_CATALOG[config.auditorModels[selectedAuditorData.id] || selectedAuditorData.defaultModel]
											if (!m) return null
							...
							<div className="pt-2">
								...
								<textarea
									...
									value={generateFullPrompt(
										selectedAuditorData,
										config.auditorPersonalities[selectedAuditorData.id] || selectedAuditorData.defaultPersonality
									)}
								/>
							</div>
						...
						<div className="pt-4 border-t dark:border-gray-700">
							<button
								...
								type="button"
								className={`w-full py-3 px-4 rounded-lg font-medium transition-colors ${config.selectedAuditors.includes(selectedAuditorData.id)
