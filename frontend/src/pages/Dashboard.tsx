import { ArrowRightIcon, FileTextIcon, LightbulbIcon, RocketIcon, SearchIcon } from 'lucide-react'
		...
		<div className="border-2 border-blue-200 dark:border-blue-700 rounded-lg p-6 bg-blue-50 dark:bg-blue-900/20">
			<div className="text-center space-y-4">
				...
				<p className="text-sm opacity-80 max-w-lg mx-auto">
					The LLM Council will automatically research your market, analyze competitors, define the
					problem, identify the industry, and create a complete vision using YC-style best
					practices.
				</p>
				<div className="space-y-4 max-w-lg mx-auto">
					<div>
						<label htmlFor={ideaInputId} className="block text-sm font-medium mb-2 text-left">
							Describe Your Idea
						</label>
						<textarea
							id={ideaInputId}
						...
						/>
						<div id={`${ideaInputId}-counter`} className="text-xs opacity-60 text-right mt-1">
			...
			<div className="border rounded-lg p-6 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20">
				<div className="text-center space-y-4">
					...
					<h2 className="text-xl font-semibold">
						Council is Working on "{projectName || generateProjectName(ideaText)}"
					</h2>
					<p className="text-sm opacity-80 max-w-lg mx-auto">
						Our AI council is researching your idea, analyzing the market, and preparing
					...
					{[
						{
							role: 'Research Agent',
							llm: 'Tavily + GPT-4',
							activity: 'Gathering market intelligence and competitor analysis',
							status: 'active',
						},
						{
							role: 'PM Auditor',
							llm: 'GPT-4o',
							activity: 'Defining problem-solution fit and YC frameworks',
							status: 'active',
						},
						{
							role: 'Data Auditor',
							llm: 'Gemini Pro',
							activity: 'Identifying success metrics and evaluation frameworks',
							status: 'pending',
						},
						{
							role: 'Security Auditor',
							llm: 'Claude 3.5',
							activity: 'Assessing security requirements and compliance',
							status: 'pending',
						},
						{
							role: 'Infrastructure Auditor',
							llm: 'Grok-2',
							activity: 'Planning technical architecture and scalability',
							status: 'pending',
						},
						{
							role: 'Cost Auditor',
							llm: 'GPT-4o',
							activity: 'Estimating development costs and resource needs',
							status: 'pending',
						},
					].map((member) => (
						<div key={member.role} className="flex items-center gap-3 p-3 border rounded">
							<div
								className={`w-3 h-3 rounded-full ${member.status === 'active' ? 'bg-green-500 animate-pulse' : 'bg-gray-300'}`}
							/>
			...
			<div className="border-2 border-green-200 dark:border-green-700 rounded-lg p-6 bg-green-50 dark:bg-green-900/20">
				<div className="text-center space-y-4">
					...
					<div className="flex justify-center gap-3">
						<button
							type="button"
							className="border border-green-600 text-green-700 hover:bg-green-100 px-4 py-2 rounded transition-colors"
						>
							Review Documents
						</button>
						<button
			...
			<div className="flex justify-center">
				<div className="flex items-center gap-3">
					<div className="flex items-center">
						<div
							className={`w-3 h-3 rounded-full ${currentStep === 'idea' ? 'bg-blue-600' : 'bg-green-500'}`}
						/>
					...
					<div className="flex items-center">
						<div
							className={`w-3 h-3 rounded-full ${currentStep === 'processing' ? 'bg-blue-600 animate-pulse' : currentStep === 'complete' ? 'bg-green-500' : 'bg-gray-300'}`}
						/>
					...
					<div className="flex items-center">
						<div
							className={`w-3 h-3 rounded-full ${currentStep === 'complete' ? 'bg-blue-600' : 'bg-gray-300'}`}
						/>
