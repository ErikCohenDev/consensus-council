...
import { z } from 'zod'
import type { WebSocketMessage, NotificationMessage } from '@shared/types/core'
...
export class WebSocketService {
  ...
  private handleMessage(event: MessageEvent): void {
    try {
      ...
      this.notifyErrorHandlers(new Error(`Message parsing failed: ${error instanceof Error ? error.message : String(error)}`))
    }
  }
