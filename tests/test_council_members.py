...
class TestCouncilDebate:
    ...
    async def test_council_member_respond_to_peer(self, mock_completion):
        ...
        response = await pm_member.respond_to_peer_feedback(peer_feedback, "vision")
...
class TestCouncilConsensusBuilding:
    ...
    def test_consensus_emergence_detection(self):
        ...
        assert len(consensus_analysis.stable_agreements) >= 2
    ...
    def test_question_generation_for_alignment(self):
        ...
        assert any("delivery" in q.lower() and "analysis" in q.lower() for q in questions)
        assert any("evaluation" in q.lower() and "implementation" in q.lower() for q in questions)
